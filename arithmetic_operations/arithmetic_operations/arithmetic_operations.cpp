...
int main() {
	...
	// Это происходит из-за того, что операция 
	// div возвращает в несколько регистров результаты работы
	// в dx возвращается остаток от деления.
	// То есть программа работает для всех значений, которые
	// unsigned short способен хранить без переполнения
	__asm {
